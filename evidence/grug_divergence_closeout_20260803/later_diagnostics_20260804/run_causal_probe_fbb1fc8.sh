...
run_probe() {
  case "$MODE" in
    preflight)
      localization_rank=0
      localization_microbatch=0
      ;;
    headline)
      localization_rank=22
      localization_microbatch=54
      ;;
    *)
  ...
  exec "$PY" "$DRIVER" \
    ...
    --objective localize \
    --localization-rank "$localization_rank" \
    --localization-microbatch "$localization_microbatch" \
    --localization-token-index 8010 \
    --localization-causal-layer 2 \
...
test "$(sha256sum "$DRIVER" | cut -d' ' -f1)" = 4f0d7a468558849a5567d96e1c05d49fcc67d55df085ce82773265cab6481373
test "$(sha256sum "$WORKER" | cut -d' ' -f1)" = cced801b807d5d75ca15b7fc1e81c83121e83ecddba2cbe0807b663fb1cce0eb
