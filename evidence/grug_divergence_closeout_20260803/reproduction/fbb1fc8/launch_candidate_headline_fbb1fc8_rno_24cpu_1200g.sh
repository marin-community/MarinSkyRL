...
RUNNER="$PRODUCT_ROOT/.agents/tmp/run_candidate_headline_fbb1fc8.sh"
...
SUBMIT_DIR=$(mktemp -d /tmp/grug-correctness-headline-fbb1fc8.XXXXXX)
...
cp "$RUNNER" "$SUBMIT_DIR/run_candidate_headline_fbb1fc8.sh"
...
"$IRIS_BIN" --config "$IRIS_CONFIG" job run \
  ...
  --timeout 86400 \
  --no-wait \
  --job-name grug-paired-eager-grouped-fbb1fc8-s1-rno-20260803 \
  --task-image "$IMAGE" \
  -e IMAGE "$IMAGE" \
  -e HF_HOME /hf/cache \
  -e HF_HUB_OFFLINE 1 \
  ...
  -- \
  /bin/bash /app/run_candidate_headline_fbb1fc8.sh
