...
def save(role: str, value: dict) -> None:
    ...
    (output / f"{role}-result.json").write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
...
class Store:
    def __init__(self, root: str):
        ...
        self.client = boto3.client(
            ...
            config=Config(s3={"addressing_style": "virtual"}, max_pool_connections=16, tcp_keepalive=True),
        )
    ...
    def get(self, sample: str, index: int, deadline: float) -> tuple[bytes, dict]:
        while time.monotonic() < deadline:
            try:
                ...
                return body, {"etag": response.get("ETag"), "headers": response["ResponseMetadata"].get("HTTPHeaders", {})}
...
def receiver(store: Store, max_seconds: int) -> None:
    ...
    with (
        ...
        for connection, port in ((listener, PORT), (data_listener, PORT + 1)):
...
def send_tcp(connections: list[socket.socket], pieces: list[bytes], streams: int, buffer_bytes: int) -> list[dict]:
    def send_stream(stream: int) -> dict:
        ...
        return {"send_buffer_bytes": actual_buffer, "send_seconds": time.monotonic() - started, "tcp_info": tcp_info(connection)}
...
def sender(
    ...
    with socket.create_connection((host, PORT), timeout=30) as control:
        ...
        result["tcp_connections"] = [
            {"local_ip": connection.getsockname()[0], "peer_ip": connection.getpeername()[0]} for connection in connections
        ]
        save("sender", result)
        try:
