...
def save(result: dict) -> None:
    ...
    suffix = f"-{result['rank']}" if result["world_size"] > 2 else ""
    (directory / f"{result['role']}{suffix}-result.json").write_text(
...
def main() -> None:
    ...
    parser.add_argument("--rank", type=int, help="Defaults to rank 1 for sender and 0 for receiver")
    ...
    rank = args.rank if args.rank is not None else (1 if args.role == "sender" else 0)
    if not 0 <= rank < args.world_size or not 0 <= args.source_rank < args.world_size:
        parser.error("Rank and source rank must be within world size")
    if (args.role == "sender") != (rank == args.source_rank):
        parser.error("The sender must be the source rank")
    if rank and not args.host:
        parser.error("--host is required for all nonzero ranks")
    ...
    try:
        ...
        dist.init_process_group(
            "nccl",
            init_method=f"tcp://{host}:{args.port}",
            rank=rank,
            world_size=args.world_size,
        ...
        for repeat in range(args.repeats):
            ...
            started = time.monotonic()
            for tensor in tensors:
                dist.broadcast(tensor, src=args.source_rank)
            torch.cuda.synchronize()
            actual = digest(tensors)
            gathered = [None] * args.world_size
            dist.all_gather_object(gathered, actual)
            elapsed = time.monotonic() - started
            if len(set(gathered)) != 1 or (expected and actual != expected):
                raise ValueError(f"SHA-256 mismatch: {gathered}")
            result["samples"].append(
                {
                    "repeat": repeat,
                    "sha256": actual,
                    "acknowledged_seconds": elapsed,
                    "receiver_sha256": gathered[0],
                    "sender_sha256": gathered[args.source_rank],
                    "rank_sha256": gathered,
                }
            )
            save(result)
        result["success"] = True
        ...
        for path in glob.glob("/tmp/cross-zone-qual-nccl.*.log"):
            shutil.copyfile(path, Path(os.environ["IRIS_OUTPUT_DIR"]) / f"{args.role}-{rank}-{Path(path).name}")
        save(result)
        print(
            json.dumps({"role": args.role, "success": result.get("success"), "samples": len(result["samples"])}),
            flush=True,
        )
