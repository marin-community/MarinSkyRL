...
def main() -> None:
    ...
    try:
        ...
        for repeat in range(args.repeats):
            if rank:
                tensor = torch.randint(0, 256, (result["bytes"],), dtype=torch.uint8, device="cuda")
                expected = digest(tensor)
            else:
                tensor = torch.empty(result["bytes"], dtype=torch.uint8, device="cuda")
                expected = None
            torch.cuda.synchronize()
            started = time.monotonic()
            dist.broadcast(tensor, src=1)
            torch.cuda.synchronize()
            actual = digest(tensor)
            gathered = [None, None]
