...
Records are canonical JSON compressed with deterministic gzip settings. One finalized runner batch is stored as a deterministic ZIP archive containing the selected `.json.gz` records and a manifest. This replaces hundreds of serial object-store PUTs with one archive PUT plus one ledger PUT. The sink reserves archive overhead and checks the final archive size against per-step and per-run budgets. Content-derived record and archive IDs make resumed or retried writes idempotent.
