...
Records are canonical JSON compressed with deterministic gzip settings. One finalized runner batch is stored as a deterministic ZIP archive containing the selected `.json.gz` records and a manifest. This replaces hundreds of serial object-store PUTs with one archive PUT plus one ledger PUT. The sink takes records in deterministic priority order until their compressed payloads reach the remaining per-step or per-run target. ZIP and manifest overhead do not affect admission, so the published archive can exceed the configured target by its container overhead. Content-derived record and archive IDs make resumed or retried writes idempotent.
