...
Records are canonical JSON compressed with deterministic gzip settings. One finalized runner batch is stored as a deterministic ZIP archive containing the selected `.json.gz` records and a manifest. This replaces hundreds of serial object-store PUTs with one archive PUT plus one ledger PUT. The sink computes the exact ZIP layout while selecting records and checks it against the per-step and per-run budgets. The final encoded size must match that projection. Content-derived record and archive IDs make resumed or retried writes idempotent.
