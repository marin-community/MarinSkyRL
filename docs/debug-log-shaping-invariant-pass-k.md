...
- Restricted `unshaped_rewards` to one scalar verifier outcome per trajectory; token-level shaping remains confined to the optimization reward channel.
- Added a trainer-boundary assertion for the published `reward/avg_pass_at_2` metric, in addition to the shared metric-function regression.
