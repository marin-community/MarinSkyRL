...
`rl_data.py` invokes `python -m cloud.iris.extract_tasks_from_parquet` for a Hugging Face dataset ID. The runtime manifest includes `rl_data.py` and `hf_datasets.py`, but omits the invoked module and its local `tasks_parquet` dependency.
...
Add a regression test that builds the real runtime workspace, disables site packages, supplies stubs for optional third-party imports, and invokes the extractor's `--help` entrypoint. This exercises module loading without downloading or reading a dataset.
