...
DEFAULT_BUNDLE_ROOT = Path("/Users/benjaminfeuer/Documents/experiments/active/iris-job-bundles")
...
def parse_regex_filters(specifications: Sequence[str], allowed_fields: Collection[str]) -> list[RegexFilter]:
    ...
    for specification in specifications:
        ...
        if normalized_field not in allowed:
            raise ValueError(f"Unknown filter field {normalized_field!r}; choose from {', '.join(sorted(allowed))}.")
        try:
...
def format_duration(
    ...
    *,
    ...
    if started_at_ms is None:
        return "—"
    ended_at_ms = (
        finished_at_ms if finished_at_ms is not None else now_ms if now_ms is not None else int(time.time() * 1000)
    )
...
def _column_widths(headers: Sequence[str], rows: Sequence[Sequence[StyledCell]], max_width: int) -> list[int]:
    ...
    minimums = [
        min(natural_width, max(10, min(_display_width(header), 12))) for header, natural_width in zip(headers, natural)
    ]
...
def box_table(
    ...
    *,
    ...
    rendered_rows = [[value if isinstance(value, StyledCell) else StyledCell(value) for value in row] for row in rows]
...
def render_error_report(title: str, checked_at: datetime, errors: Sequence[MonitorError]) -> str:
    ...
    for scope in sorted(grouped):
        ...
        for error in sorted(grouped[scope], key=lambda item: (item.operation, item.message)):
            lines.append(f"- **{_single_line(error.operation)}:** {_single_line(error.message)}")
        lines.append("")
    return "\n".join(lines)
    ...
    while True:
        ...
        if max_polls is not None and polls >= max_polls:
            ...
            return (
                snap
                if snap is not None
                else JobStateSnapshot(job_id=job_id, state="unspecified", state_int=0, source="timeout")
            )
        time.sleep(interval)
...
def main() -> int:
    ...
    ap.add_argument(
        "--cluster", default=DEFAULT_CLUSTER, help=f"iris cluster (default: {DEFAULT_CLUSTER}; use 'marin' for TPU)"
    )
    ...
    try:
        if args.once:
            ...
            snap = watch(
                args.job_id, args.cluster, interval=args.interval, check_pods=check_pods, max_polls=args.max_polls
            )
    except RuntimeError as e:
