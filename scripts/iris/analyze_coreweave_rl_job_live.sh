...
PYBIN=$(kexec 'for p in python python3 /opt/*/envs/rl/bin/python /opt/*/.venv/bin/python /usr/local/bin/python3 /usr/bin/python3; do ("$p" -c "import boto3" >/dev/null 2>&1) && { echo "$p"; break; }; done' 2>/dev/null | tr -d '\r' | head -1)
