...
def run_verified_all_gather(
    ...
    input_values = torch.arange(total_values, device=device, dtype=torch.int64) + rank * RANK_VALUE_STRIDE
    output_values = torch.empty(total_values * len(group_ranks), dtype=input_values.dtype, device=device)
    dist.all_gather_into_tensor(output_values, input_values, group=group)
    offsets = torch.arange(total_values, device=device, dtype=torch.int64)
    expected_values = torch.cat([offsets + source_rank * RANK_VALUE_STRIDE for source_rank in group_ranks])
...
def run_verified_all_to_all(
    ...
    expected_values = torch.cat(
        [offsets + source_rank * RANK_VALUE_STRIDE + group_rank * values_per_peer for source_rank in group_ranks]
    )
    torch.testing.assert_close(output_values, expected_values)
