...
register(
    id="test_env",
    entry_point="tests.gpu.gpu_ci.test_skyrl_gym_runner:TestEnv",
)
...
async def run_trajectory_runner_end_to_end(
    ...
    outputs = [
        {
            "response": trajectory_batch["response_ids"][i],
            "loss_mask": trajectory_batch["loss_masks"][i],
        }
        for i in range(len(trajectory_batch["response_ids"]))
    ]
    ...
    if max_turns == 1:
        ...
        assert trajectory_batch["rollout_metrics"]["generate/max_num_tokens"] <= max_generate_length
...
async def test_trajectory_runner_formatting_use_conversation_multi_turn(model_name):
    ...
    try:
        ...
        for i, resp_ids in enumerate(trajectory_batch["response_ids"]):
            ...
            stop_reason = trajectory_batch["stop_reasons"][i]
            ...
            if stop_reason == "stop":
                ...
                # Either make TrajectoryBatch return stop reason for each turn, or change the way we manage
