...
`trainer.policy.fsdp_config.use_grouped_mm=true` replaces the eager loop over 256 experts with
grouped matrix multiplies. `test_grouped_gemm_parity.py` covers the generic HF `moe_grouped_gemm`
swap and never sets `use_grouped_mm=True`. This file covers the native Grug path:
`GrugMoeSparseMoeBlock.enable_grouped_mm`, `_GrugGroupedExpertExecution` and
`_run_experts_grouped_mm`.

Gates:
  G4a-1  padded tail rows are zeroed. ``torch._grouped_mm`` writes only the rows covered by
         ``offs`` and leaves the ALIGN_SIZE_M padding uninitialized (pytorch#186365). moe.py zeroes
         it; this asserts that directly, with no tolerance.
  G4a-2  enable_grouped_mm engages on every block. Without this check a no-op would pass every
         parity test, because eager would be compared with itself.
  G4a-3  eager vs grouped forward parity on identical weights and inputs.
  G4a-4  eager vs grouped backward parity.
  G4a-5  the grouped forward gives identical results across repeats. Reading uninitialized memory
         would not, and the check needs no tolerance.
  G4a-6  the combine gives identical results while another stream contends for the device. This is
         the only test that reaches the CUDA atomics the fixed-order combine replaced.

Run on two H100s with ``ci/run_fsdp2_train_eval_parity.sh``, which also runs the FSDP2 gate.
"""
...
def test_g4a_1_grouped_mm_zeroes_the_padded_tail_rows():
    """moe.py zeroes the padded tail that torch._grouped_mm leaves uninitialized (pytorch#186365)."""
    # Call the undecorated impl. The @expert_parallel wrapper runs generate_permute_indices first
    # and expects num_tokens_per_expert in the EP-sharded 128-wide layout, so a plain per-expert
    # count vector fails in torchtitan with "shape '[0, -1]' is invalid". The impl is also where
    # the pytorch#186365 mitigation lives. Its argument order is (w1, w2, w3, x, counts).
    torch.manual_seed(3)
...
def test_g4a_2_enable_grouped_mm_engages_on_every_block():
    ...
    assert blocks, "no Grug MoE blocks found; the fixture built the wrong model"
