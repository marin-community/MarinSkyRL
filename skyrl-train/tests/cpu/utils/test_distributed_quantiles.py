...
def test_unequal_shards_and_microbatches_match_exact_float32_oracle(seed):
    ...
    assert actual["p50"] == pytest.approx(oracle[0].item(), abs=1e-12)
    assert actual["p95"] == pytest.approx(oracle[1].item(), abs=1e-12)
...
def test_nonfinite_coverage_ties_extremes_and_replica_ownership():
    ...
    assert actual["p95"] == pytest.approx(oracle.item(), rel=1e-12)
    assert actual["p50"] == 1
    assert calls == 5
...
def test_empty_shards_and_single_value():
    ...
    assert actual["p50"] == actual["p95"] == 3.25
    assert actual["quantiles_valid"] == 1
    assert calls == 5
