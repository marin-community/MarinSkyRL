...
`grug_muonh_jax_golden.npz` was generated with the pinned Marin production
implementation at commit
`61a4a8c81dc6d9171c02ea0490bc941bdf2de238`.
...
The fixture uses seed `20260730`, float32 parameters and gradients, three
deterministic steps, shared MuonH/AdamH LR `0.03`, Adam LR `0.004`, momentum
`0.95`, Nesterov enabled, five quintic Newton--Schulz steps, betas
`(0.9, 0.95)`, and both epsilons `1e-8`. Marin's outer `weight_decay` was set
to `123.0` to verify that it cannot affect this recipe. Matrices are transposed
over their final two axes when saved so the arrays have PyTorch layout.

To regenerate, archive the pinned commit, put the archive root and its Levanter
source tree first on `PYTHONPATH`, and run the script below from this fixture
directory. The script contains the complete parameter tree, route keys, state
extraction, layout conversion, archive write, and checksum calculation.

```sh
PINNED_MARIN=/tmp/marin-grug-muonh-61a4a8c
mkdir -p "$PINNED_MARIN"
git -C ~/dev/marin archive 61a4a8c81dc6d9171c02ea0490bc941bdf2de238 | tar -x -C "$PINNED_MARIN"
PYTHONPATH="$PINNED_MARIN:$PINNED_MARIN/lib/levanter/src" \
  uv run --project ~/dev/marin --no-sync python /tmp/generate_grug_muonh_golden.py
sha256sum grug_muonh_jax_golden.npz
```

Save this as `/tmp/generate_grug_muonh_golden.py`:
...
OUTPUT = Path("grug_muonh_jax_golden.npz")
STEPS = 3
...
for step_index in range(STEPS):
    ...
    for identifier, path in paths.items():
        ...
        if routes[identifier] == "muonh":
            ...
            fixture[f"adam_mu_{step_index + 1}__{identifier}"] = torch_layout(
                identifier, get(adam_state.mu, path)
            )
            fixture[f"adam_nu_{step_index + 1}__{identifier}"] = torch_layout(
                identifier, get(adam_state.nu, path)
            )
