...
Gates (scope §6):
  G4-1        create_device_mesh(4, 2, timeout_seconds=10, ep_size=2) → shape (1, 2, 2) with dim names
              ["ddp", "fsdp", "ep"]; divisibility asserts fire on bad combos.
...
def _mesh_worker(rank, world_size):
    with gloo_process_group(rank, world_size, 29555):
        ...
        assert mesh.mesh_dim_names == ("ddp", "fsdp", "ep"), f"names={mesh.mesh_dim_names}"
