...
from skyrl_train.distributed import collective_count_diag
...
def test_phase_record_includes_world_and_mesh_group_sequences(monkeypatch):
    ...
    monkeypatch.setattr(collective_count_diag, "_default_process_group", lambda: world)

    region_id = collective_count_diag.begin_region(
        mesh,
        kind="policy_training_step",
        rank=2,
        metadata={"global_step": 3, "local_step": 7},
    )
    with patch.object(collective_count_diag.logger, "info") as info:
        record = collective_count_diag.log_phase("model_forward_enter")
    ...
    parsed = collective_count_diag.parse_log_line(info.call_args.args[0])
...
def test_moe_boundary_guard_crosses_asyncio_to_thread_and_resets_by_phase(monkeypatch):
    ...
    region_id = collective_count_diag.begin_region(mesh, kind="policy_training_step", rank=0)
    messages: list[str] = []
    with patch.object(collective_count_diag.logger, "info", messages.append):
        collective_count_diag.log_phase("model_forward_enter", reset_moe_boundary=True)

        async def record_forward_boundary() -> None:
            await asyncio.to_thread(collective_count_diag.log_moe_ep_boundary_once)
            await asyncio.to_thread(collective_count_diag.log_moe_ep_boundary_once)

        asyncio.run(record_forward_boundary())
        collective_count_diag.log_phase("backward_enter", reset_moe_boundary=True)
        collective_count_diag.log_moe_ep_boundary_once()
        collective_count_diag.log_moe_ep_boundary_once()

    records = [collective_count_diag.parse_log_line(message) for message in messages]
...
def test_disabled_diagnostics_do_not_read_process_groups(monkeypatch):
    ...
    with patch.object(collective_count_diag.logger, "info") as info:
        assert collective_count_diag.begin_region(mesh, kind="policy_training_step", rank=0) is None
        assert collective_count_diag.log_phase("model_forward_enter") is None
        assert collective_count_diag.log_moe_ep_boundary_once() is None
...
def test_capture_failures_warn_without_interrupting_training(monkeypatch):
    ...
    monkeypatch.setattr(collective_count_diag, "_default_process_group", lambda: FakeProcessGroup(1))
    collective_count_diag.begin_region(mesh, kind="policy_training_step", rank=0)

    with patch.object(collective_count_diag.logger, "warning") as warning:
        assert collective_count_diag.log_phase("backward_enter") is None

    assert "backward_enter" in warning.call_args.args[0]
    assert "coordinate lost" in warning.call_args.args[0]
...
def _record(
    ...
    *,
    ...
    fsdp: int,
    ep: int,
) -> collective_count_diag.CollectivePhaseRecord:
    return collective_count_diag.CollectivePhaseRecord(
        region_id=4,
...
def test_first_divergence_reports_the_process_group_and_rank_pair():
    ...
    divergence = collective_count_diag.find_first_divergence(records)
    ...
    assert divergence.expected == 21
    assert divergence.actual == 22
...
def test_first_divergence_reports_a_rank_that_never_reaches_the_boundary():
    ...
    assert divergence.actual == "<missing>"
...
def test_matching_phase_records_have_no_divergence():
    ...
    assert collective_count_diag.find_first_divergence(records) is None
