...
def _recording_observer(tracker):
    """Keep what the strategy handed the observer; the worker stores it the same way."""
    observed = []

    def observe(*args, **kwargs):
        observed.append(tracker.observe(*args, **kwargs))
        return observed[-1]
...
def test_fsdp_observer_sees_clipped_gradients_before_optimizer_clears_them():
    ...
    observe, observed = _recording_observer(tracker)
    for step in range(2):
        model.weight.grad = torch.tensor([[3.0, 4.0]])
        raw = strategy.optimizer_step(optimizer, model, None, grad_observer=observe)
        assert raw == 5
        assert model.weight.grad is None
        assert observed[-1]["grad_norm_reduced"] == pytest.approx(1.0, abs=1e-6)
        assert observed[-1]["grad_cosine_valid"] == step
    model.weight.grad = torch.full_like(model.weight, float("nan"))
    strategy.optimizer_step(optimizer, model, None, grad_observer=observe)
    assert tracker.previous is None
    assert observed[-1]["grad_norm_valid"] == 0
    model.weight.grad = torch.tensor([[3.0, 4.0]])
    strategy.optimizer_step(optimizer, model, None, grad_observer=observe)
    assert observed[-1]["grad_cosine_valid"] == 0
...
def test_megatron_native_hook_window_retains_postclip_shards_and_resets_skips():
    ...
    for index in range(2):
        assert step(strategy, optimizer, None, scheduler, grad_observer=observe) == 5
        assert observed[-1]["grad_norm_reduced"] == pytest.approx(1.0)
        assert observed[-1]["grad_cosine_valid"] == index
        assert optimizer.grad.abs().sum() == 0
    optimizer.successful = False
    step(strategy, optimizer, None, scheduler, grad_observer=observe)
...
def test_direction_range_covers_only_comparisons_that_had_a_predecessor():
    updates = [
        {"grad_cosine": 0.0, "grad_cosine_valid": 0.0},
        {"grad_cosine": 0.7, "grad_cosine_valid": 1.0},
        {"grad_cosine": -0.2, "grad_cosine_valid": 1.0},
    ]
    assert gradient_direction_summary(updates) == {"grad_cosine_min": -0.2, "grad_cosine_max": 0.7}
    # A single update has no predecessor to compare against, so it reports no range
    # rather than a range whose endpoints are the same invented point.
    assert gradient_direction_summary(updates[:1]) == {}
