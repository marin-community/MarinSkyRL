...
def test_pre_child_receipt_is_durable_before_command_and_failure_preserved(tmp_path, monkeypatch):
    ...
    command = [sys.executable, "-c", f"import json; assert json.load(open({str(receipt.with_name("bootstrap-before-child.json"))!r}))['state']=='before_child'; print('original failure'); raise SystemExit(7)"]
...
def test_actual_wrapper_captures_blocked_import_stack_before_runtime(tmp_path):
    ...
    command = [sys.executable, "-c", "import sys; sys.path.insert(0, sys.argv[1]); from skyrl_train.entrypoints.startup_capture import run_module; run_module('blocked_import', [], stack_seconds=.02)", str(tmp_path)]
...
def test_late_upload_failure_preserves_child_exit(tmp_path, monkeypatch):
    ...
    assert capture([sys.executable, "-c", "raise SystemExit(9)"], str(tmp_path), "failure", interval=.01) == 9
