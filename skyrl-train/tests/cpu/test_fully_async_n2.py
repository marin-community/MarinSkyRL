...
class _BaseConstructorReached(Exception):
    pass
...
@pytest.mark.parametrize("updates_per_cohort", [1, 2, 4])
def test_qualified_cohort_geometries_reach_base_constructor(monkeypatch, updates_per_cohort):
    def stop_before_resource_initialization(*args, **kwargs):
        raise _BaseConstructorReached

    cfg = get_default_config()
    cfg.trainer.train_batch_size = 32 * updates_per_cohort
    ...
    monkeypatch.setattr(
        "skyrl_train.fully_async_trainer.RayPPOTrainer.__init__",
        stop_before_resource_initialization,
    )

    with pytest.raises(_BaseConstructorReached):
        FullyAsyncRayPPOTrainer(cfg=cfg)
