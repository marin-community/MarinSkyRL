"""`combine_routed_rows` sums each token's rows in float32, in ascending expert order.

It replaced a `scatter_add` over repeated token indices, whose atomic adds ran in a different order
on each launch. The tests below use rows whose float32 sum depends on the order of the adds.
"""
...
def test_combine_routed_rows_sums_each_token_in_ascending_expert_order():
    """Rows whose sum is exact only when added in ascending expert order.

    Per token t, in ascending expert order, the rows are ``big, 1, -big, 3 + t`` with ``big`` at
    least 2**24, so ``big + 1`` rounds back to ``big`` in float32 and the chain gives exactly
    ``3 + t``. Of the 24 possible orders, two give that result on every token: the two that add the
    ``1`` to ``big`` before the pair cancels. Two more match on every token except t=0.

    This test checks the order of the adds and catches a misaligned reshape. It cannot catch a
    return to ``scatter_add``: on CPU that kernel adds in source order, which the stable sort
    preserves within a token, so both give identical results. The CUDA atomics are covered only by
    the GPU gates. The accumulator dtype is covered by
    ``test_combine_routed_rows_is_exact_when_the_rows_share_a_scale`` and
    ``test_grug_moe.py::test_bfloat16_sparse_moe_forward_uses_float32_accumulation``, because every
    value in this fixture is exact in bf16.
    """
    num_tokens = 6
