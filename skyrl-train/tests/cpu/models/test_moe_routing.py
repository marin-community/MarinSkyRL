...
def test_combine_routed_rows_backward_returns_each_tokens_gradient_to_every_route(dtype):
    num_tokens = 13
    _, token_indices, _ = _routing(num_tokens, seed=19)
    rows = torch.randn(num_tokens * TOP_K, HIDDEN, dtype=dtype, requires_grad=True)
    token_gradients = torch.arange(num_tokens * HIDDEN, dtype=torch.float32).reshape(num_tokens, HIDDEN)

    combined = combine_routed_rows(rows, token_indices, num_tokens, TOP_K)
    (combined * token_gradients).sum().backward()

    # Each routed row contributes once to its token, independently of expert order.
    expected = token_gradients.index_select(0, token_indices).to(dtype)
    torch.testing.assert_close(rows.grad, expected, rtol=0, atol=0)
