"""A Ray worker actor owns its own process telemetry.

``main_base`` enters process telemetry for the trainer and driver only; a worker actor is a
separate process, and rigging discards every record while unconfigured. The learner memory,
optimizer inventory and Megatron timing rows all originate in the worker.
"""
