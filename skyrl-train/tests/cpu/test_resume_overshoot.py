...
def _make_bare_trainer(cls, global_step: int, total_training_steps: int, colocate_all: bool = False):
    ...
    trainer._stage_checkpoint = MagicMock(name="stage_checkpoint")
    trainer._commit_checkpoint = MagicMock(name="commit_checkpoint")
...
def test_train_end_saves_the_last_completed_step(cls):
    ...
    saved_steps = []
    trainer._stage_checkpoint.side_effect = lambda: saved_steps.append(trainer.global_step)
    ...
    trainer._commit_checkpoint.assert_called_once()
...
def test_train_end_still_saves_when_the_final_evaluation_fails(cls):
    ...
    trainer._stage_checkpoint.assert_called_once()
    trainer._commit_checkpoint.assert_called_once()
    assert trainer.callback_handler.events == ["on_train_end", "on_save"]
...
def test_handle_resume_at_max_steps_no_save_when_not_requested(cls):
    ...
    trainer._stage_checkpoint.assert_not_called()
    trainer._commit_checkpoint.assert_not_called()
