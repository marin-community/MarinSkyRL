...
def _make_bare_trainer(cls, global_step: int, total_training_steps: int, colocate_all: bool = False):
    ...
    trainer._write_checkpoint_continuation_state = MagicMock(name="write_checkpoint_continuation_state")
...
def test_train_end_saves_the_last_completed_step(cls):
    ...
    saved_steps = []
    trainer._save_checkpoint_payloads.side_effect = lambda: saved_steps.append(trainer.global_step)
