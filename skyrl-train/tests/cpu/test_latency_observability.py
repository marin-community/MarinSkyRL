...
def test_tracker_forwards_only_finite_numeric_metrics_with_step(monkeypatch):
    recorder = _Recorder()
    nonfinite = _Recorder()
    monkeypatch.setattr(trainer_telemetry, "training_metric", recorder)
    monkeypatch.setattr(trainer_telemetry, "training_nonfinite_values", nonfinite)

    trainer_telemetry.record_training_metrics(
    ...
    assert recorder.calls == [
        (1.25, {"metric": "loss", "step": "7", "role": "trainer", "payload_kind": "train"}),
        (3.0, {"metric": "count", "step": "7", "role": "trainer", "payload_kind": "train"}),
    ]
    assert len(nonfinite.calls) == 2
