...
def bound_sender(completed_update):
    ...
    sender.stream = SimpleNamespace(run=lambda version, **kwargs: InstallReport(0, version, 2, 48, 0.01))
    return sender


def test_sends_the_update_it_finished_and_the_loaded_weights_before_any_update():
    assert (
        bound_sender(completed_update=4).send_weights({"version": 4, "sparse": False, "encoding": "dense"})["version"]
        == 4
    )
    assert (
        bound_sender(completed_update=None).send_weights({"version": 9, "sparse": False, "encoding": "dense"})[
            "version"
        ]
        == 9
    )
...
def test_refuses_a_version_that_is_not_the_completed_update():
    with pytest.raises(RuntimeError, match="names update 5 but this rank last completed 4"):
        bound_sender(completed_update=4).send_weights({"version": 5, "sparse": False, "encoding": "dense"})
...
def test_refuses_when_a_parameter_was_reassigned_new_storage_since_preparation():
    ...
    assert sender.send_weights({"version": 1, "sparse": False, "encoding": "dense"})["expert_matrices"] == 2
    ...
    with pytest.raises(RuntimeError, match="storage changed"):
        sender.send_weights({"version": 1, "sparse": False, "encoding": "dense"})
