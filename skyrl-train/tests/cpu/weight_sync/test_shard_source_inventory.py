...
def test_complete_schedule_installs_current_expert_views_on_every_replica(dp_count, replicas):
    trainers = tuple(
        TrainerRank(i, dp, pp, ep) for i, (dp, pp, ep) in enumerate(product(range(dp_count), range(2), range(2)))
    )
    receivers = tuple(
        ReceiverRank(i, replica, ep) for i, (replica, ep) in enumerate(product(range(replicas), range(2)))
    )
    states = {}
    entries = {}
...
def test_dense_stream_covers_qkv_shared_router_bytes_with_single_landing_per_replica(dp_count, replicas):
    ...
    installed = {
        r.rank: {
            name: torch.zeros(shape, dtype=getattr(torch, dtype)) for name, (shape, dtype) in expected_shapes.items()
        }
        for r in receivers
    }
    coverage = set()
    for transfer in plan:
        ...
        for landing, local_ranks in zip(transfer.landing_native_ranks, transfer.replica_fanout, strict=True):
            ...
            for rank in local_ranks:
                target = (
                    installed[rank][transfer.source.hf_name]
                    .view(-1)
                    .narrow(0, transfer.source.hf_offset, transfer.source.numel)
                )
                target.copy_(source)
    ...
    for values in installed.values():
        for pp in range(2):
            torch.testing.assert_close(
                values[f"model.layers.{pp}.self_attn.q_proj.weight"],
                torch.tensor([[0, 1, 2], [6, 7, 8]], dtype=torch.bfloat16) + 16 * pp,
            )
            assert values[f"model.layers.{pp}.mlp.router.bias"].view(torch.int32)[0].item() == -(2**31)
    assert len(coverage) == len(receivers) * sum(
        torch.tensor(shape).prod().item() for shape, _ in expected_shapes.values()
    )
    broken = list(rows)
