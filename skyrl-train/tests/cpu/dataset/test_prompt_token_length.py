...
def test_actual_chat_token_lengths_filter_parquet(tmp_path):
    ...
    tokenizer.chat_template = (
        "{% for m in messages %}{{m['content']}} {% endfor %}{% if add_generation_prompt %}answer{% endif %}"
    )
    ...
    assert [
        tokenizer.apply_chat_template(actual[i][0], add_generation_prompt=True, return_dict=False)
