...
def _build_runner(
    ...
    """Helper to create a SkyRLGymTrajectoryRunner with common config/env settings."""
...
def _serving_engine(tokenizer, response_text: str, *, stop_reason: str, policy_version: int):
    """An engine that serves the chat prompt and returns its own prompt ids, sampled ids, logprobs, spans and routes."""
    ...
    def generate(input_batch):
        ...
        return {
            ...
            "routed_experts": [[[[index, index + 1]] for index in range(len(sampled_ids))]],
        }
...
async def test_skyrl_gym_runner_chat_templating_exact(model_name, tokenization_codepath, expected_str):
    ...
    trajectory_batch: TrajectoryBatch = await runner.run(input_batch)
    ...
    resp_str = tokenizer.decode(trajectory_batch["response_ids"][0])
    trajectory_batch_str = prompt_str + resp_str
    if tokenization_codepath == "tito" and "Qwen" in model_name:
        ...
        # `<|assistant|>\Some content<|im_end|>` for trajectory_batch_str.
        if expected_str.endswith("\n"):
            expected_str = expected_str[:-1]
    assert trajectory_batch_str == expected_str
...
async def test_append_eos_after_stop_multi_turn(model_name, tokenization_codepath):
    ...
    async def make_runner(append_flag: bool):
        mock_llm = MagicMock()
    ...
    runner_true = await make_runner(True)
    out_true: TrajectoryBatch = await runner_true.run(
        {"prompts": prompt, "env_extras": extras, "env_classes": ["cpu_test_env"], "sampling_params": sp}
    )

    # Case 2: append flag = False
    runner_false = await make_runner(False)
    out_false: TrajectoryBatch = await runner_false.run(
        {"prompts": prompt, "env_extras": extras, "env_classes": ["cpu_test_env"], "sampling_params": sp}
    )
...
def _rendered_assistant_ids(tokenizer, runner, content: str) -> list[int]:
    """The tokens the re-render path trains on for one assistant message."""
...
async def test_single_turn_chat_trajectory_trains_on_the_engines_tokens():
    """Without an observation the trainer gets the served prompt, the sampled ids, their logprobs, spans and routes."""
    _register_test_env_if_needed()
    tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-0.6B")
...
async def test_length_stopped_single_turn_carries_no_fabricated_end_of_turn():
    ...
    # The re-render path closes the cut-off message with an end-of-turn token the model never emitted.
