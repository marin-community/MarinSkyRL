...
def test_config_absent_block_means_disabled():
    ...
    assert not config.rollout_context_forward_needed
    assert not config.reference_uses_rollout_context
...
def test_config_base_defaults_are_off_and_match_the_terminus_layout():
    ...
    assert (config.on_failure, config.tis_reference, config.kl_reference) == ("mask", "rollout", "rollout")


def test_config_enabled_defaults_need_both_extra_forwards():
    config = ContextDistillationConfig.from_algorithm_config(
        OmegaConf.create({"context_distillation": {"enabled": True}})
    )
    assert config.enabled
    assert config.rollout_context_forward_needed
    assert config.reference_uses_rollout_context
    lean = ContextDistillationConfig(enabled=True, tis_reference="none", kl_reference="training")
    assert not lean.rollout_context_forward_needed
    assert not lean.reference_uses_rollout_context
...
def test_validate_cfg_reports_bad_choices_and_step_wise_training():
    with initialize_config_dir(config_dir=config_dir, version_base=None):
        bad = compose(
            config_name="ppo_base_config", overrides=["trainer.algorithm.context_distillation.tis_reference=x"]
        )
        step_wise = compose(
            config_name="ppo_base_config",
            overrides=["trainer.algorithm.context_distillation.enabled=true", "trainer.step_wise_training=true"],
        )
    with pytest.raises(ValueError, match="tis_reference"):
        validate_cfg(bad)
    with pytest.raises(ValueError, match="step_wise_training"):
        validate_cfg(step_wise)
