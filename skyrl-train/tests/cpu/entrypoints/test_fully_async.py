...
def test_no_experiment_anywhere_in_the_tree_overrides_the_base_run():
    """Examples and scripts launch the same way, so an override there exports no telemetry either.

    The registered-entrypoint test above imports its modules; this one reads source instead, so it
    also covers trees whose imports CPU CI cannot satisfy.
    """
    offenders = {}
