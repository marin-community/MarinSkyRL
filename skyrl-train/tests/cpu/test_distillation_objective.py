...
from skyrl_train.distillation import (
    ...
    sampled_reverse_kl_input_from_tensors,
...
def test_topk_teacher_evidence_validates_as_a_distinct_transport_variant():
    request = replace(_request(), evidence=TeacherEvidenceKind.TOPK_DISTRIBUTION)
...
def test_partial_distillation_payload_fails_closed():
    with pytest.raises(ValueError, match="requires teacher logprobs, valid_mask, and loss_weights together"):
        sampled_reverse_kl_input_from_tensors(
            torch.tensor([[-0.5]]),
            None,
            torch.ones(1, 1),
