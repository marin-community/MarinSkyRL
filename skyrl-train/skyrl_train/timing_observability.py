...
TIMING_PARENTS: dict[str, str | None] = {
    ...
    "checkpoint_foreground_pause": "step",
    "checkpoint_pointer_lag": "step",
    "save_checkpoints": "checkpoint_foreground_pause",
...
def local_directory_bytes(path: str) -> int | None:
    """Measure staged checkpoint/export bytes without touching remote storage."""
    try:
        return sum(
        ...
        logger.opt(exception=True).warning("Could not measure local checkpoint bytes")
        return None
...
def checkpoint_phase(
    ...
    *,
    ...
    """Publish a rank-local, machine-readable checkpoint wall span without a CUDA sync."""
    ...
    try:
        ...
        try:
            ...
            # Instrumentation must never turn a recoverable checkpoint into a failed one.
