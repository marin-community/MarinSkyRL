...
def configure_tis_logprobs(generator: DictConfig) -> None:
    ...
    for key, required in (("logprobs_mode", "processed_logprobs"), ("generation_config", "vllm")):
    ...
    OmegaConf.update(options, "logprobs_mode", "processed_logprobs", force_add=True)
    # Do not inherit checkpoint penalties or filters that the trainer does not apply.
    OmegaConf.update(options, "generation_config", "vllm", force_add=True)
    OmegaConf.update(options, "enforce_tis_sampling", True, force_add=True)
