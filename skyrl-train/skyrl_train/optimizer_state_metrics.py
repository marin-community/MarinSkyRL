"""Inventory one worker's Megatron Adam state after its first successful update.

The inventory reads tensor metadata before zero_grad. It does not call state_dict,
which Transformer Engine can upcast, and it does not copy tensors, synchronize
CUDA or reset allocator peaks. Category byte totals can overlap through aliases;
only the all-category storage union adds up within a worker, and neither counts
CUDA or NCCL allocations outside the observed storages. master_state is Transformer
Engine's raw master_param state (int16 residuals when remainders are enabled) or
the native optimizer's FP32 parameter shard. A tensor view counts its whole
backing storage. The inventory's host work adds to the first optimizer step's
timing.
"""
...
class OptimizerStateObserver:
    ...
    A failure logs an error and disables this observer; the optimizer step is unaffected.
    """
