"""DeepEP dispatch/combine backend — Stage 5 (perf swap behind ``ep_comm_backend="deepep"``).

Near-verbatim port of prime-rl ``src/prime_rl/trainer/distributed/deepep.py``. The
``deep_ep`` API in this env (``deep_ep 1.2.1+73b6ea4``, JSC vLLM-kernel aarch64/cu13
build) matches prime-rl's upstream 1.2.1 (verified in ``stage5_deepep_scope.md`` §1):
``Buffer.{dispatch,combine,get_dispatch_layout,get_dispatch_config,get_combine_config,
set_num_sms}`` + ``deep_ep.utils.{EventHandle,EventOverlap}`` + the
``torch.library``/``torch.compiler`` autograd-registration surface all present.

This module is **lazy-imported** — only ``MoE._run_deepep_routed_experts`` (the
``ep_comm_backend=="deepep"`` branch) imports it, so the base / torch-EP / a3 paths
never touch ``deep_ep`` or NVSHMEM. ``register_deepep_cuda_ops()`` runs at module
import (i.e. first deepep use), registering the ``deepep::dispatch`` custom op + its
autograd (combine on backward); ``_DeepEPCombine`` carries the combine's autograd
(re-dispatch on backward). Together dispatch→combine is invertible (token i → row i),
which is what preserves router replay under DeepEP (scope §3).
"""
