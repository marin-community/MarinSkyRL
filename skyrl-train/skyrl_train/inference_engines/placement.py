...
def node_local_bundle_nodes(
    ...
    """Return each replica's node after checking its bundles are complete and on one node."""
    node_ids = []
...
def verified_inference_replica_placements(
    ...
    *,
    ...
    for index, (report, offset) in enumerate(zip(reports, relative_rank_offsets, strict=True)):
        if len(report) != 1:
            raise ValueError("Node-local TP=PP=1 requires one worker report per DP actor")
