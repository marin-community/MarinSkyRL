...
async def install_and_replay(worker, client, *, source_owners):
    ...
    try:
        ...
        for replay in (False, True):
            ...
            for bucket in range(manifest.bucket_count):
                ...
                if rank == 0:
                    receive_task = asyncio.create_task(
                        client.receive_diagnostic_weight_sync_bucket(bucket, replay=replay)
                    )
            ...
            if rank == 0:
                method = (
                    client.finish_diagnostic_weight_sync_replay
                    if replay
                    else client.finish_diagnostic_weight_sync_install
                )
                ...
                for index, row in enumerate(receivers):
                    ...
                    if replay:
                        ...
                        range(min(2, manifest.bucket_count))
                    ):
            ...
            if replay and peak_extra > MAX_REPLAY_EXTRA_BYTES:
                raise ValueError("Sender replay introduced more than 1 MiB of additional allocation")
            phases["replay" if replay else "install"] = {
                "seconds": elapsed,
