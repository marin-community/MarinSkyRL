...
The caller owns the learner's frozen interval, including the second export used
for byte replay. This packer retains one complete exported tensor at a time; it
never requests incomplete bridge conversion groups.
"""
...
class StreamingBucketSender:
    ...
    def pack_next_bucket(self):
        ...
        self.next_bucket += 1
        return buffer.narrow(0, 0, entries[-1].offset + entries[-1].nbytes)
