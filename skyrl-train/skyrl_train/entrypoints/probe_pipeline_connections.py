"""One fresh-process cell of the CUDA connection/allocation diagnostic.
...
def validate_pair(rows, connections, allocation):
    """Qualify bytes and the serial control while preserving zero pipeline overlap."""
    assert len(rows) == 4
    ...
    for row in rows:
        assert row["connections"] == connections and row["allocation"] == allocation
...
def run_case(mode, device, connections, allocation):
    rank = dist.get_rank()
    ...
    # Pool allocation precedes the measured region and its completion barrier.
    # Its deliberate full-source retention is synthetic only, not a production memory proposal.
    pool = (
        {spec.name: torch.empty(spec.shape, dtype=torch.bfloat16, device=device) for spec in specs}
        if rank == 0 and allocation == "preallocated"
        else {}
    )
    dist.barrier()
    ...
    if rank == 0:
        ...
        def exports():
            for i, spec in enumerate(specs):
                ...
                if allocation == "preallocated":
                    source = pool[spec.name].fill_(i + 1)
                else:
                    source = torch.full(spec.shape, i + 1, dtype=torch.bfloat16, device=device)
    ...
    return {
        ...
        "source_pool_bytes": sum(t.numel() * t.element_size() for t in pool.values()),
        "rank": rank,
...
def main():
    ...
    parser.add_argument("--connections", type=int, choices=(1, 8), required=True)
    parser.add_argument("--allocation", choices=("streaming", "preallocated"), required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    assert os.environ.get("CUDA_DEVICE_MAX_CONNECTIONS") == str(args.connections)
    ...
    try:
        ...
        for mode in ("serial", "pipeline"):
            local = run_case(mode, device, args.connections, args.allocation)
            gathered = [None, None]
            dist.all_gather_object(gathered, local)
            rows.extend(gathered)
            if rank == 0:
                print(
                    "K9_CONNECTION_CASE_RECEIPT " + json.dumps({"source_commit": args.source_commit, "rows": gathered}),
                    flush=True,
                )
        if rank == 0:
            print(
                "K9_CONNECTION_CELL_RECEIPT " + json.dumps({"source_commit": args.source_commit, "rows": rows}),
                flush=True,
            )
            validate_pair(rows, args.connections, args.allocation)
            args.output.write_text(json.dumps({"source_commit": args.source_commit, "rows": rows}) + "\n")
            print(
                f"K9_CONNECTION_CELL_PASS connections={args.connections} allocation={args.allocation} byte_completion=true serial_control=true overlap_is_observation=true",
                flush=True,
            )
    finally:
