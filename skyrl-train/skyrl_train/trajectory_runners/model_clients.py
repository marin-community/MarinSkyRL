            ...
            if chat_options is not None:
                results = await asyncio.gather(
                    *(
                        ...
                        for messages, session_id, options in zip(prompts, session_ids, chat_options, strict=True)
                    )
                )
                logprobs = [result.response_logprobs for result in results]
                selected_indices = [result.student_topk_indices for result in results]
    ...
    async def _generate_exact_one(
        ...
        *,
        ...
        async with session.post(
            ...
            json={"model": self._model_name, "messages": messages, **tokenize_options, "add_generation_prompt": True},
        ) as response:
            tokenization = await response.json()
            if response.status >= 400:
                raise RuntimeError(f"OpenAI chat tokenization returned HTTP {response.status}: {tokenization}")
        prompt_ids = tokenization.get("tokens")
        if not isinstance(prompt_ids, list) or not all(isinstance(token, int) for token in prompt_ids):
            raise RuntimeError(f"OpenAI chat tokenization did not return exact prompt token IDs: {tokenization}")
