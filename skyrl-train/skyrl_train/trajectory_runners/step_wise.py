"""
This file implements per-transition SkyRL-Gym collection for step-wise projection.
"""
...
class StepWiseRolloutCollector:
    ...
    async def agent_loop(
        ...
        """
        retokenize_chat_history = self.trajectory_runner_cfg.get("retokenize_chat_history", False)
        ...
        while not done:
            ...
            if oldest_version is None:
                oldest_version = oldest_policy_version(engine_output.get(RESPONSE_POLICY_VERSION_SEGMENTS_KEY, []))
        ...
        if per_step_outputs:
            per_step_outputs[0].oldest_policy_version = oldest_version
