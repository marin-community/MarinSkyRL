...
def load_verified_replay(
    ...
    headline_shards = WORLD_SIZES["headline"]
    if manifest["batch"]["batch_size"] != HEADLINE_SEQUENCES or len(manifest["shards"]) != headline_shards:
        raise RuntimeError(f"headline replay must contain {HEADLINE_SEQUENCES} rows in {headline_shards} rank shards")
...
def make_config(args: argparse.Namespace, sequence_length: int):
    ...
    cfg.trainer.policy.fsdp_config.fsdp_size = -1
...
def make_rank_batches(
    manifest: dict[str, Any], shards: list[dict[str, torch.Tensor]], mode: str
) -> list[TrainingInputBatch]:
    selected = shards[:8] if mode == "preflight" else shards
    batches = []
    for shard in selected:
        data = {name: shard[name][:1].contiguous() for name in TENSOR_FIELDS} if mode == "preflight" else dict(shard)
        for name in NONE_FIELDS:
            data[name] = None
        batch = TrainingInputBatch(data)
        batch.metadata = dict(manifest["batch_metadata"])
...
def assert_topology(topology: list[dict[str, Any]], mode: str) -> None:
    ...
    if len(by_host) != expected_hosts or {len(items) for items in by_host.values()} != {GPUS_PER_NODE}:
        raise RuntimeError(f"expected {expected_hosts} complete {GPUS_PER_NODE}-GPU hosts, got {by_host}")
    for node_index in range(expected_hosts):
        group = topology[node_index * GPUS_PER_NODE : (node_index + 1) * GPUS_PER_NODE]
...
def upload_result(client, uri: str, result: dict[str, Any]) -> str:
    """Add the canonical result digest, upload JSON, and return the payload digest."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="grug-fixed-replay-") as work:
        ...
        batches = make_rank_batches(manifest, shards, args.mode)
        del shards
        global_loss_tokens = sum(int(batch["loss_mask"].sum().item()) for batch in batches)
        ...
        try:
            ...
            for identity in identities:
                ...
                if identity["expert_parallel_size"] != 1 or identity["grouped_moe"]:
                    raise RuntimeError(f"unsupported expert path selected: {identity}")
                ...
                expected_mini = MICROBATCHES_PER_RANK[args.mode]
            ...
            if args.objective == "operational":
                for item in timed:
                    ...
                    if not all(is_finite_number(value) for value in status.values()):
                ...
                for item, identity in zip(timed, identities):
                    ...
                    if args.expert_attribution:
                        ...
                        if evidence["call_counts"] != {
                            "forward": expected_calls,
                            "backward": expected_calls,
                        }:
            ...
            logical_sequences = int(manifest["batch"]["batch_size"]) if args.mode == "headline" else world_size
            metrics = {
