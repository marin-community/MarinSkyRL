...
.. note::
    ...
    The non-agentic ``fully_async`` entrypoint uses the direct ``InferenceEngineClient`` through
    ``SkyRLGymTrajectoryRunner``, preserving engine token IDs and behavior logprobs. It does not require an HTTP
    endpoint or a custom chat template. Harbor uses its separate entrypoint and runner.
    Use ``trainer.step_wise_training=false`` for this whole-trajectory path; the separate step-wise collector's
    completion-time version stamp has not been qualified for the conservative freshness contract below.
...
- Inherit ``get_trajectory_runner()`` to use the same direct SkyRL-Gym runner as synchronous training. Its native
  token-in/token-out path preserves behavior logprobs required by ``behavior_clip``. A custom retokenizing template
  discards those logprobs and is unsuitable for that objective.
...
.. code-block:: bash
    ...
    uv run --project .. --extra fsdp --extra vllm -m skyrl_train.entrypoints.fully_async \
    ...
...
- ``max_staleness_steps = S``: the maximum allowed difference between the published policy version available before
  an optimizer update and the earliest submission version of any sample in its group. SkyRL-Gym captures the last
  published version before the first inference request, preserving that conservative bound if generation crosses
  a weight update. Initial weights have version 0; a successful weight publication advances it. This bound does
  not identify an exact version for every sampled token.
  The existing ``actual_global_step`` / ``earliest_model_step`` fields encode this publication bound plus one,
  matching the trainer's one-based target-step coordinate: ``global_step - earliest_model_step`` is the same
  difference as ``(global_step - 1) - publication_bound``.
