...
- ``trainer.step_wise_training``: Whether to use step-wise training. If ``true``, each model turn becomes a separate training sequence containing the exact prompt, generated response tokens, and rollout log probabilities for that turn. Observation tokens appear in the next turn's prompt rather than the preceding response. The final turn's outcome advantage is propagated to every turn using each turn's own response mask. Step-wise training therefore supports only outcome-based advantage estimators; ``gae`` and ``reinforce++`` are rejected. ``policy_mini_batch_size`` and ``critic_mini_batch_size`` continue to count prompts, so a variable number of turns does not change the number of optimizer updates. Harbor workloads additionally require ``terminal_bench_config.harbor.collect_rollout_details=true`` and ``terminal_bench_config.harbor.enable_summarize=false``. Set ``generator.merge_step_wise_output=true`` to compact consecutive turns whose token streams share an exact prefix; inserted observation tokens remain loss-masked. Prefix compaction is incompatible with routed-expert capture and with enabled trajectory reward shaping.
