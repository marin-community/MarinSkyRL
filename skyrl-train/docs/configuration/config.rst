...
- ``generator.weight_sync_transport``: How weights travel to the engines. ``broadcast`` (default) sends every tensor from trainer rank 0 to every engine. ``expert_block`` broadcasts each MoE expert matrix from the Megatron rank that owns it straight into the live parameter slot of the vLLM worker that serves it, and dense weights from a stage's owners to one worker per replica with a node-local fan-out. It requires a Grug MoE trained with Megatron at ``tensor_model_parallel_size: 1`` and the same ``expert_model_parallel_size`` as ``generator.inference_engine_expert_parallel_size``, local async vLLM engines with TP=PP=1 placed with ``inference_engine_node_local: true``, and vLLM's TRITON MoE backend; anything else is refused at configuration or preparation time.
...
- ``generator.inference_engine_node_local``: Opt-in. Place each engine's whole DP/EP replica on one node and verify at startup that its workers run there on distinct physical GPUs; a replica that does not fit or does not match is killed before training starts. Requires non-colocated async vLLM with TP=PP=1 and EP=DP. Default ``false`` keeps today's placement for every configuration.
