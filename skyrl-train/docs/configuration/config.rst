...
- ``generator.inference_engine_node_local``: Place each pipeline stage of an engine's DP/EP replica on one node, so its expert all-to-all stays on NVLink, and verify at startup that its workers run there on distinct physical GPUs; a replica whose workers do not match is killed before training starts. Default ``true``. Applies to non-colocated async vLLM engines on the Ray executor with TP=1, DP>1, EP=DP and a replica stage that fits one node; any other engine keeps the default placement, and the log says why. ``false`` never packs replicas. Independent per-replica groups can leave nodes partly used, as per-engine groups already do for TP/PP>1 engines; ``trainer.placement.policy_strict_spread_pg`` reserves the policy's nodes first.
