...
- ``generator.weight_sync_transport``: How weights travel to the engines. ``broadcast`` (default) sends every tensor from trainer rank 0 to every engine. ``expert_block`` broadcasts each MoE expert matrix from the Megatron rank that owns it straight into the live parameter slot of the vLLM worker that serves it, and dense weights from a stage's owners to one worker per replica with a node-local fan-out. It requires a Grug MoE trained with the ``megatron`` strategy at ``tensor_model_parallel_size: 1`` and ``expert_tensor_parallel_size: 1``, local async vLLM engines placed as node-local replicas (``inference_engine_node_local``, the default at TP=1 with EP=DP>1), ``weight_sync_backend: nccl`` and vLLM's TRITON MoE backend. The trainer's and the engines' expert-parallel degrees may differ (each must divide the expert count) and the engines may be pipeline-parallel. It runs under ``FullyAsyncRayPPOTrainer`` only; any other configuration is refused at configuration or preparation time. Each sync logs ``timing/expert_block_sync/{install,policy,receiver,expert,dense}_seconds``.
- ``generator.expert_block_sync.timeout_seconds``: Timeout for creating the expert-block sync groups at startup and for each sync's collectives.
- ``generator.expert_block_sync.verify``: If set, replay each sync and verify it against the trainer's values, failing the run on any mismatch. Costs about one extra sync per update.
...
- ``generator.inference_engine_node_local``: Whether each pipeline stage of an engine's DP/EP replica is placed on one node, so its expert all-to-all stays on NVLink, with a startup check that its workers run there on distinct physical GPUs (a replica whose workers do not match is killed before training starts). An engine can hold such a replica when it is non-colocated async vLLM on the Ray executor with TP=1, DP>1, EP=DP and a replica stage that fits a node. ``auto`` (default) packs such an engine when its own placement group cannot leave nodes partly used, that is, when the cluster has one GPU node or the engine's DP×PP GPUs are a whole number of nodes; any other engine is placed as before and the log says why. ``require`` packs any engine that can hold a replica, including one smaller than a node, and refuses the run otherwise; independent groups smaller than a node can strand the policy group, which ``trainer.placement.policy_strict_spread_pg`` guards against. ``off`` never packs.
