...
- ``generator.expert_block_sync.encoding``: Routed-expert wire encoding. ``dense`` (default) uses the existing full-tensor transfer. ``sparse_index`` sends changed BF16 values with flat int32 indices in approximately 128 MiB buckets after a dense first publication. Shared and non-expert weights remain dense. A failed sparse publication requires restart or dense reseeding because receiver state may be partial. Sparse patches do not repair unrelated receiver drift; enable ``verify`` to detect it. Sparse sync adds ``timing/expert_block_sync/{changed_density,logical_dense_bytes,encoded_bytes,expert_buckets,expert_collectives,detect_seconds,pack_seconds,transfer_seconds,apply_seconds}``.
