...
- ``generator.weight_sync_transport``: How weights travel to the engines. ``broadcast`` (default) sends every tensor from trainer rank 0 to every engine. ``expert_block`` broadcasts each MoE expert matrix from the Megatron rank that owns it straight into the live parameter slot of the vLLM worker that serves it, and dense weights from a stage's owners to one worker per replica with a node-local fan-out. It requires a Grug MoE trained with the ``megatron`` strategy at ``tensor_model_parallel_size: 1`` and ``expert_tensor_parallel_size: 1``, local async vLLM engines at TP=1 placed with ``inference_engine_node_local: true``, ``weight_sync_backend: nccl`` and vLLM's TRITON MoE backend. The trainer's and the engines' expert-parallel degrees may differ (each must divide the expert count) and the engines may be pipeline-parallel; a tensor-parallel trainer is refused (the schedule can land TP shards as regions, but a TP2/ETP1 Megatron trainer showed its expert replicas diverging across the TP pair after one update); anything else is refused at configuration or preparation time. Each sync logs ``timing/expert_block_sync/{install,policy,receiver,expert,dense}_seconds``.
...
- ``generator.expert_block_sync.verify``: Qualification gate, off by default and not for production. After every sync, replay it and fail unless every receiver's installed bytes match, every installable parameter byte was covered and every trainer rank agrees byte for byte with its data-parallel peers. Costs about one extra sync on the wire per update.
...
- ``generator.inference_engine_node_local``: Opt-in. Place each pipeline stage of an engine's DP/EP replica on one node and verify at startup that its workers run there on distinct physical GPUs; a replica that does not fit or does not match is killed before training starts. Requires non-colocated async vLLM with TP=1 and EP=DP. Default ``false`` keeps today's placement for every configuration.
