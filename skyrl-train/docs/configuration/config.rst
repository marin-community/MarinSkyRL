...
- ``generator.expert_block_sync.verify``: If set, replay each synchronization and verify it against the trainer's values, failing the run on any mismatch. Costs about one extra sync per update.
