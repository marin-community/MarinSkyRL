...
runtime exports them before Ray starts so its actors inherit them. Inheriting
them is not enough on its own: Rigging discards records in a process that never
configured it, so the trainer and driver enter the export lifecycle in the
entrypoint and every worker actor enters it in its constructor.
