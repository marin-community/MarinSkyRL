...
Usage:
    harbor trials start -p <task-dir> -a oracle \\
        --environment-import-path marinskyrl.iris_harbor_environment:IrisEnvironment

Inside an Iris job, ``IRIS_CONTROLLER_URL`` selects the same controller by
default. Local callers may instead pass exactly one of ``cluster`` or
``controller_url``.
"""
...
class IrisEnvironment(BaseEnvironment):
        ...
        **kwargs,
        ...
        Args:
            ...
            controller_url: Direct controller URL, bypassing cluster config.
                Defaults to ``IRIS_CONTROLLER_URL`` when neither connection
        ...
        if (cluster is None) == (controller_url is None):
            raise ValueError(
                "IrisEnvironment requires exactly one of `cluster` or `controller_url`; "
                "IRIS_CONTROLLER_URL supplies the latter inside an Iris job."
            )
    ...
    def _start_sync(self) -> None:
        # The endpoint owns any tunnel or local resources needed to reach the
        # controller, and therefore stays open until _stop_sync finishes.
        ...
        try:
            ...
            self._job = self._iris.submit(
                ...
                name=self._job_name(),
                # setup_scripts=[] runs the prebuilt task image as-is.
                ...
                # retry must start a fresh environment instead.
    ...
    def _stop_sync(self) -> None:
        try:
            ...
            self._rpc = None
            try:
                iris, self._iris = self._iris, None
                if iris is not None:
                    iris.shutdown()
            finally:
                # Close the endpoint last because the client may reach the
                # controller through a tunnel owned by it.
    ...
    async def download_file(self, source_path: str, target_path: Path | str):
        ...
        if target is None:
            ...
            with tempfile.NamedTemporaryFile() as staging:
                ...
                UPath(str(target_path)).write_bytes(Path(staging.name).read_bytes())
            return
