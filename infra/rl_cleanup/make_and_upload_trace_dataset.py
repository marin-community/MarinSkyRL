...
- No flag (default)  -> stream-upload, THEN register the finished HF repo in the campaign registry.
- ``--skip_register``  -> stream-upload only, no registry registration (RL path).

Examples:

  # Datagen: export, upload (streaming), AND register in the campaign registry
...
def _install_safe_episode_guard() -> None:
    ...
    def safe_find_episode_dirs(trial_dir: Path):
        ...
        if episodes_root.exists() and not episodes_root.is_dir():
            print(f"[trace-export] Skipping trial {trial_dir} because {episodes_root} is not a directory.")
            return []
        try:
...
def _install_inline_subagent_merger() -> None:
    ...
    def _append_conversation_turn(messages: List[Dict[str, str]], role: str, content: str) -> None:
    ...
    def _append_subagent_transcripts(
        ...
        for result in results:
            ...
            for ref in refs:
                ...
                formatted_messages = _format_subagent_conversation(subagent_conv, label, summary)
                for formatted in formatted_messages:
                    _append_conversation_turn(messages, formatted["role"], formatted["content"])
    ...
    def _extract_episode_with_subagents(
        ...
        for idx, step in enumerate(steps):
            ...
            if source in {"system", "user"}:
                ...
                _append_conversation_turn(conv["conversations"], "assistant", assistant_content)
                ...
                if not is_last_agent_step:
                    ...
                    if observation_text:
                        _append_conversation_turn(conv["conversations"], "user", observation_text)
...
def _finalize_chunk(ds):
    ...
    Mirrors ``infra.rl_cleanup.trace_dataset_hygiene._finalize_trace_dataset`` and
    ``data.commons.clean_empty_structs`` but is applied to a single bounded
    chunk so peak memory stays small.
    """
    try:
        from infra.rl_cleanup.trace_dataset_hygiene import _finalize_trace_dataset  # type: ignore
...
def register_trace_dataset(repo_id: str, dataset_type: str = "SFT") -> None:
    """Register an already-uploaded trace dataset repo in the campaign registry.
    ...
    try:
        ...
        raise SystemExit(
            "Could not import register_hf_dataset for registry registration. Registration is optional — pass --skip_register to upload only. "
            f"Import error: {exc}"
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--dataset_type",
        default="SFT",
        help="Dataset type label ('SFT' or 'RL') used by the optional registry step. Default: SFT",
    )
    p.add_argument(
        "--skip_register",
        action="store_true",
        help="Upload only; do NOT register the dataset in any registry. Controls ONLY "
...
def write_tokenizer_provenance(api, repo_id: str, provenance: Dict[str, Any]) -> None:
    ...
    print(
        f"[trace-export] Stamped tokenizer provenance ({served_model}) into {repo_id} ({PROVENANCE_FILENAME} + README)."
    )
...
def resolve_literal_inclusion(
    ...
    *,
    ...
    """
    from infra.rl_cleanup.literal_correlator import discover_literal_logs
...
def main() -> None:
    ...
    if include_literal_tokens:
        from infra.rl_cleanup.literal_correlator import enrich_trajectories_with_literals

        traces_utils = _import_traces_utils()
        print(f"[trace-export] Correlating literal tokens from {len(literal_logs)} file(s): {literal_logs} ...")
    ...
    # Step 2 — ORTHOGONAL post-upload registry registration, gated by --skip_register.
    if args.skip_register:
        print("[trace-export] --skip_register set: upload only, NOT registering in the campaign registry.")
    else:
        print(
            f"[trace-export] Registering {args.repo_id} in the campaign registry (dataset_type={args.dataset_type})..."
        )
        register_trace_dataset(args.repo_id, dataset_type=args.dataset_type)
        print(f"[trace-export] Registry registration complete for {args.repo_id}.")
