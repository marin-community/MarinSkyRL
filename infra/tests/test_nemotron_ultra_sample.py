...
from infra.rl_data.nemotron_ultra_sample import BlendSampleSpec, range_rows, sample_raw_generator_rows
...
def test_range_reader_discards_partial_boundary_lines(monkeypatch):
    ...
    assert range_rows("https://example.invalid/data", 100) == [complete]
...
def test_range_reader_rejects_server_that_ignores_range(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="ignored bounded JSONL range"):
        range_rows("https://example.invalid/data", 100)
...
def test_sampler_selects_one_row_per_generator(monkeypatch):
    ...
    monkeypatch.setattr("infra.rl_data.nemotron_ultra_sample.range_rows", lambda url, offset: rows)
...
def test_sampler_chooses_only_swe_rows_with_exact_tasktrove_proxies(monkeypatch):
    ...
    monkeypatch.setattr("infra.rl_data.nemotron_ultra_sample.range_rows", lambda url, offset: [row(2), row(3)])
    proxies = {
        SWEProxyKey(
            trajectory_id="7",
            step=3,
