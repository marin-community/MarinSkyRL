...
def test_smoke_sample_splits_trajectory_ids_and_writes_parquet(tmp_path, monkeypatch):
    rows = [_raw_row(1), _raw_row(1), *(_raw_row(index) for index in range(2, 14))]
    ...
    assert len(set(manifest["train_trajectory_ids"] + manifest["probe_trajectory_ids"])) == 12
    assert len(load_dataset("parquet", data_files=str(tmp_path / "train.parquet"), split="train")) == 8
    assert len(load_dataset("parquet", data_files=str(tmp_path / "probe.parquet"), split="train")) == 4
...
def test_report_pairs_probe_results_and_persists_training_errors(tmp_path):
    ...
    archive_dir = retention_root / "schema_v3" / "archives" / "phase=train" / "step=00000001"
    ...
    training = json.loads((diagnostics_root / "training.jsonl").read_text())
    ...
    assert comparison["after"]["output_response"] == "action-1"
    assert training["disposition"]["exception_type"] == "TimeoutError"
