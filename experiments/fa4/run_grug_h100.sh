...
world_size="${1:?usage: run_grug_h100.sh WORLD_SIZE SHAPE [STEPS] [capture]}"
shape="${2:?usage: run_grug_h100.sh WORLD_SIZE SHAPE [STEPS] [capture]}"
steps="${3:-1}"
capture="${4:-}"
...
if [[ "$capture" == "capture" ]]; then
    common_args+=(--capture-grads)
elif [[ -n "$capture" ]]; then
    echo "unknown capture mode: $capture" >&2
    exit 2
fi
