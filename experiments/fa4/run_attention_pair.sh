...
batch="${1:?usage: run_attention_pair.sh BATCH SEQ HEADS KV_HEADS HEAD_DIM WINDOW [WARMUPS] [SAMPLES]}"
seq="${2:?usage: run_attention_pair.sh BATCH SEQ HEADS KV_HEADS HEAD_DIM WINDOW [WARMUPS] [SAMPLES]}"
heads="${3:?usage: run_attention_pair.sh BATCH SEQ HEADS KV_HEADS HEAD_DIM WINDOW [WARMUPS] [SAMPLES]}"
kv_heads="${4:?usage: run_attention_pair.sh BATCH SEQ HEADS KV_HEADS HEAD_DIM WINDOW [WARMUPS] [SAMPLES]}"
head_dim="${5:?usage: run_attention_pair.sh BATCH SEQ HEADS KV_HEADS HEAD_DIM WINDOW [WARMUPS] [SAMPLES]}"
window="${6:?usage: run_attention_pair.sh BATCH SEQ HEADS KV_HEADS HEAD_DIM WINDOW [WARMUPS] [SAMPLES]}"
warmups="${7:-3}"
samples="${8:-10}"
...
common_args=(
    ...
    --head-dim "$head_dim" --window-left "$window"
